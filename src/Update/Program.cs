...
namespace Squirrel.Update
{
    ...
    class Program : IEnableLogger
    {
            ...
            using (var logger = new SetupLogLogger(isUninstalling) { Level = Splat.LogLevel.Info })
            {
                ...
                switch (updateAction)
                {
                    ...
                    case UpdateAction.Update:
                        var updateTask = Update(target, maximumReleaseDate);
                        updateTask.Wait();

                        var updateInfo = updateTask.Result;
                        if (updateInfo.ReleasesToApply.Count == 0)
                        {
                            // There are either no updates *or* they are outside the maintenance window
                            if (updateInfo.HasReleasesAfterMaximumDate)
                            {
                                exitCode = ExitCodes.NewVersionAvailableButNewerThanMaximumDate;
                            }
                            else
                            {
        ...
        public async Task<UpdateInfo> Update(string updateUrl, DateTime maximumReleaseDate, string appName = null)
        {
            ...
            retry:
                ...
                try
                {
                    updateInfo = await mgr.CheckForUpdate(ignoreDeltaUpdates, maximumReleaseDate, x => Console.WriteLine(x / 3));
